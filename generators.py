        ...
        if(self.siren.is_siren):
            ...
            with torch.no_grad():
                ...
                for b in range(batch_size):
                    ...
                    while head < transformed_points.shape[1]:
                        tail = head + max_batch_size
                        coarse_output[b:b+1, head:tail] = self.siren.forward(transformed_points[b:b+1, head:tail], z[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail]) #def forward(self, input, z, ray_directions, **kwargs)
                        head += max_batch_size
                ...
                if hierarchical_sample:
                    ...
                    for b in range(batch_size):
                        ...
                        while head < fine_points.shape[1]:
                            tail = head + max_batch_size
                            fine_output[b:b+1, head:tail] = self.siren.forward(fine_points[b:b+1, head:tail], z[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
                            head += max_batch_size
